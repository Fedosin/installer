...
func ValidatePlatform(p *openstack.Platform, n *types.Networking, fldPath *field.Path, fetcher ValidValuesFetcher, c *types.InstallConfig) field.ErrorList {
	...
	if p.MachinesSubnet != "" && !validUUIDv4(p.MachinesSubnet) {
		allErrs = append(allErrs, field.Invalid(fldPath.Child("machinesSubnet"), p.MachinesSubnet, "invalid custom subnet ID"))
	}
