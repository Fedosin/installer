...
* `machinesSubnet` (optional string): the UUID of an openstack subnet to install the nodes of the cluster onto. If you set `network.machineNetwork`, the cidr must be equivilent to the `machinesSubnet`. If you do not set `network.machineNetwork`, it will automatically be set to the CIDR of the `machinesSubnet` by the installer. Also note that setting `externalDNS` while setting `machinesSubnet` is invalid usage. If you want to add a DNS to your cluster while using a custom subnet, add it to the subnet in openstack [like this](https://docs.openstack.org/neutron/rocky/admin/config-dns-res.html).
